...
# Handle when the FinalBoss is hit by an attack
func _on_hurtbox_area_entered(area: Area2D) -> void:
	print("FinalBoss hit by:", area.name)  # Debugging: Check what hit the boss
	...
	# Play the "poof" sound effect when the FinalBoss dies
	poofSound.play()

	# Wait for the death animation to finish and then remove the boss
